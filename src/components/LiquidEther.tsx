...
export function LiquidEther({
  ...
  useEffect(() => {
    ...
    class Simulation {
      ...
      getFloatType() { 
        // HalfFloat is generally sufficient and faster
      ...
      dispose() {
        ...
        this.advection?.plane?.geometry?.dispose();
        this.advection?.material?.dispose();
        this.externalForce?.mouse?.geometry?.dispose();
        const mouseMat = this.externalForce?.mouse?.material;
        if (mouseMat) {
          if (Array.isArray(mouseMat)) mouseMat.forEach(m => m.dispose());
          else mouseMat.dispose();
        }
        this.viscous?.material?.dispose();
        this.divergence?.material?.dispose();
        this.poisson?.material?.dispose();
        this.pressure?.material?.dispose();
      }
    ...
    class Output {
      ...
      dispose() {
        ...
        (this.output.material as THREE.RawShaderMaterial).dispose();
      }
    }
